...
install:
	...
	test -f `echo $$fx | sed 's/.so//' | sed 's/mda.lv2\//mda.lv2\/modgui\//' | tr A-Z a-z`* && cp `echo $$fx | sed 's/.so//' | sed 's/mda.lv2\//mda.lv2\/modgui\//' | tr A-Z a-z`* $(INSTALL_PATH)/mda-`echo $$fx | sed 's/build\/mda.lv2\///' | sed 's/.so//' | tr A-Z a-z`.lv2/modgui/ || true ;\
	done
	@cd ..
